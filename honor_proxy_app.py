...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "adv_perf_hint":     {"EN": "💡 Faster scan: use 16–32 threads + enter start/end ID + larger step (10–50)",
                          "HU": "💡 Gyorsabb scan: 16–32 szál + konkrét ID tartomány + nagyobb lépés (10–50)"},
    "btn_search":        {"EN": "🔍  Search",                    "HU": "🔍  Keresés"},
    "btn_direct_id":     {"EN": "🎯  Test exact ID",             "HU": "🎯  Konkrét ID tesztelése"},
    "direct_id_label":   {"EN": "Test single ID:",               "HU": "Konkrét ID tesztelése:"},
...
class CDNClient:
    ...
    _timeout = 5  # Gyorsabb timeout scanning-hoz
    _last_request_time = 0  # Rate limiting
    _min_request_interval = 0.05  # 50ms közlözés a kérések között

    def _get(self, url: str, timeout: int = None, retry: int = 2) -> bytes:
        """HTTP/HTTPS + retry logic + rate limiting."""
        if timeout is None:
            timeout = self._timeout
        
        # Rate limiting - elkerüli a Honor szerverek túltöltödését
        import time
        elapsed = time.time() - CDNClient._last_request_time
        if elapsed < CDNClient._min_request_interval:
            time.sleep(CDNClient._min_request_interval - elapsed)
        CDNClient._last_request_time = time.time()
        urls_to_try = [url]
            ...
            for attempt in range(retry + 1):
                try:
                    ...
                    if attempt < retry:
                        ...
                        continue
                    # 404 = normális scanning közben (nincs verzió) - ne logoljunk
                    if "404" not in str(e):
                        qlog("debug", f"Fetch error ({u}, attempt {attempt+1}): {e}")
                    break
    ...
    def fetch_filelist(self, vid: str) -> list[FirmwareEntry]:
        ...
        try:
            ...
            # Parsing error csak valódi hiba logolva (nem "empty response")
            if raw and len(raw) > 10:  # Ha volt adat de parse hiba
        ...
        if not file_nodes:
            # Silent skip - normális, hogy nincs firmware verzió
            return []
    ...
    def scan_versions(self, fw_version, build_code,
        ...
        # Thread warning
        if threads > 64:
            qlog("warn", f"⚠ {threads} szál túl sok! A CDN rate-limitelit + memória pazarlás. Max 32-64 ajánlott.")
            threads = min(threads, 64)  # Cap at 64
        ...
        # CHUNK dinamikus: max 50, hogy ne pufferjunk túl sok Future-t memóriában
        CHUNK = min(max(threads * 2, 32), 50)
            ...
            for chunk_start in range(0, len(ids), CHUNK):
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        # Szilenciosus skip a 404-es és connection errorokra
                        if "404" not in str(e) and "refused" not in str(e).lower():
...
class App(tk.Tk):
    ...
    def _tab_proxy(self, nb):
        ...
        # ── Local Firmware Server szekció – log box ELŐTT ─────────────────────
        lfs = tk.Frame(f, bg=BG2, padx=10, pady=8)
        lfs.pack(fill=tk.X, pady=(0, 4))
        ...
        tk.Label(lfs, text=t("serve_hint"), bg=BG2, fg=FG_DIM,
                 font=("Segoe UI", 8), justify=tk.LEFT).pack(anchor=tk.W, pady=(0, 4))
        ...
        self._log_box = scrolledtext.ScrolledText(
            f, height=12, bg=BG3, fg=FG, font=MONO,
    ...
    def _tab_finder(self, nb):
        ...
        ttk.Spinbox(thr, from_=1, to=128, width=5,
                    textvariable=self._threads_var).pack(side=tk.LEFT, padx=(4, 0))
        tk.Label(thr, text="  (16–32 optimális • 64+ = lassabb)", bg=BG2, fg=FG_DIM,
                 font=("Segoe UI", 8)).pack(side=tk.LEFT)

        tk.Label(self._adv_frame, text=t("adv_hint"), bg=BG2, fg=FG_DIM,
                 font=("Segoe UI", 8)).pack(anchor=tk.W)
        tk.Label(self._adv_frame, text=t("adv_perf_hint"), bg=BG2, fg=YELLOW,
                 font=("Segoe UI", 8)).pack(anchor=tk.W, pady=(4, 0))

        # Direct ID test (bypass range search)
        sep = tk.Frame(self._adv_frame, height=1, bg=FG_DIM); sep.pack(fill=tk.X, pady=(8, 6))
        did = tk.Frame(self._adv_frame, bg=BG2); did.pack(anchor=tk.W, pady=(0, 4))
        tk.Label(did, text=t("direct_id_label"), bg=BG2, fg=FG).pack(side=tk.LEFT, padx=(0, 4))
        self._direct_id_var = tk.StringVar(value="")
        ttk.Entry(did, textvariable=self._direct_id_var, width=15).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(did, text=t("btn_direct_id"), command=self._test_direct_id).pack(side=tk.LEFT)
    ...
    def _test_direct_id(self):
        """Egy konkrét ID-t tesztel – gyors diagnózis."""
        direct_id_str = self._direct_id_var.get().strip()
        if not direct_id_str:
            messagebox.showwarning(t("warn_no_cdn_title"), "Enter an ID to test!")
            return
        try:
            vid = int(direct_id_str)
        except ValueError:
            messagebox.showerror(t("warn_no_cdn_title"), "Invalid ID – must be a number!")
            return

        qlog("info", f"Testing direct ID: {vid}")
        entries = cdn_client.fetch_filelist(str(vid))
        if not entries:
            messagebox.showinfo("Result", f"No firmware found for ID {vid}")
            return

        # Hozzáadjuk az eredményeket a treeview-hoz és az state-hez
        state.fw_list.extend(entries)
        state.save()
        for e in entries:
            self._add_to_treeview(e)

        messagebox.showinfo("Result", f"✅ Found {len(entries)} package(s) for ID {vid}\n\nYou can now select one below.")
